...
oqs::mem_cleanse(secretKeyVec);  // Securely clean memory after copying
return buffer;
  }
  ...
  Napi::Value Signature::sign(const Napi::CallbackInfo& info) {
    ...
    try {
     ...
     Napi::Buffer<byte> buffer = Napi::Buffer<byte>::New(env, signatureVec.size());
std::memcpy(buffer.Data(), signatureVec.data(), signatureVec.size());
return buffer;
